...
The command creates a blank Godot .NET consumer outside the repository. Its first managed build uses only the staged addon and no pure-project references. The later GdUnit phase compiles the host-side Resource converter and Node binding against the pure projects. It verifies Resource save/load, cached Resource identity, runtime isolation, and a Node responder exchange that is recorded, disabled, and replayed with matching causal hashes.
